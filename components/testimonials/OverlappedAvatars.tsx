...
export default function OverlappedAvatars({ 
  ...
  const handleAvatarClick = useCallback((testimonial: Testimonial, index: number, event: React.MouseEvent | React.TouchEvent) => {
    ...
    // Calculate position relative to viewport for portal rendering (escapes overflow-hidden)
    let bubblePos: { x: number; y: number };
    
    if (isMobile) {
      // On mobile: center the bubble beneath all avatars (viewport coordinates)
      const avatarContainerRect = avatarContainer.getBoundingClientRect();
      const centerX = avatarContainerRect.left + avatarContainerRect.width / 2;
      const bottomY = avatarContainerRect.bottom + 8; // 8px below avatars
      bubblePos = { x: centerX, y: bottomY };
    } else {
      // On desktop: position beneath the clicked avatar (viewport coordinates)
      const avatarRect = avatarElement.getBoundingClientRect();
      const centerX = avatarRect.left + avatarRect.width / 2;
      const topY = avatarRect.bottom;
  ...
  return (
    <div ref={outerContainerRef} className="bg-transparent pb-0 relative" style={{ zIndex: 100 }} data-testimonials-container>
